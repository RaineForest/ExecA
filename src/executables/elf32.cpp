...
void ELF32::read(string filename) {
	...
	pHeaders = (ELF32_Program_Header*) &data[correct_endian<uint32_t>(header->ph_off, getEndianness())];

	sHeaders = (ELF32_Section_Header*) &data[correct_endian<uint32_t>(header->sh_off, getEndianness())];
}
...
void ELF32::getSection(int sectionNum, uint8_t* buffer, unsigned int len) const {
	memcpy(buffer, &data[correct_endian<uint32_t>(sHeaders[sectionNum].s_offset, getEndianness())], 
		min(len, correct_endian<uint32_t>(sHeaders[sectionNum].s_size, getEndianness())));
}

int ELF32::getSectionSize(int s) {
	return correct_endian<uint32_t>(sHeaders[s].s_size, getEndianness());
}
...
void ELF32::printHeaderInfo() {
	printf("ELF Header:\n \
		...
		correct_endian<uint32_t>(header->entry, getEndianness()),
		correct_endian<uint32_t>(header->ph_off, getEndianness()),
		correct_endian<uint32_t>(header->sh_off, getEndianness()),
...
void ELF32::printProgramHeaderInfo(int p) {
	printf("%d\t%#x\t%#x\t%#x\t%#x\t%#x\t%x\t%#x\n",
		...
		correct_endian<uint32_t>(pHeaders[p].p_offset, getEndianness()),
		correct_endian<uint32_t>(pHeaders[p].p_vaddr, getEndianness()),
		correct_endian<uint32_t>(pHeaders[p].p_paddr, getEndianness()),
		correct_endian<uint32_t>(pHeaders[p].p_filesz, getEndianness()),
		correct_endian<uint32_t>(pHeaders[p].p_memsz, getEndianness()),
		correct_endian<uint32_t>(pHeaders[p].p_flags, getEndianness()),
		correct_endian<uint32_t>(pHeaders[p].p_align, getEndianness()));
}
...
void ELF32::printSectionHeaderInfo(int s) {
	printf("%#x\t%d\t%08x\t%x\t%x\t%d\t%8x\t%d\t%d\t%d\n",
		...
		correct_endian<uint32_t>(sHeaders[s].s_addr, getEndianness()),
		correct_endian<uint32_t>(sHeaders[s].s_offset, getEndianness()),
		correct_endian<uint32_t>(sHeaders[s].s_size, getEndianness()),
		correct_endian<uint32_t>(sHeaders[s].s_link, getEndianness()),
		correct_endian<uint32_t>(sHeaders[s].s_flags, getEndianness()),
		correct_endian<uint32_t>(sHeaders[s].s_info, getEndianness()),
		correct_endian<uint32_t>(sHeaders[s].s_addralign, getEndianness()),
		correct_endian<uint32_t>(sHeaders[s].s_entsize, getEndianness()));
}
